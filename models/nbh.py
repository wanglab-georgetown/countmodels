    ...
    def _stan_code(self):
        stan_code = """
            ...
            model{
                ...
                # likelihood
                for (i in 1:N) Y[i] ~ neg_binomial_2(a[i], b) T[1,];
            }
            generated quantities {
              real log_lik;
              vector[N] log_a;
              ...
              for (i in 1:N){
                real a = exp(log_a[i]) ;
                log_lik += neg_binomial_2_lpmf(Y[i] | a, b) - log1m_exp(neg_binomial_2_lpmf(0 | a, b));
              }
            }
            """
        # the above code is a robust implementation of the following line
        # log_lik += neg_binomial_2_lpmf(Y[i] | a, b) - log(1-exp(neg_binomial_2_lpmf(0 | a, b)));
        return stan_code
